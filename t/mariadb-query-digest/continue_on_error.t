...
$output = `$trunk/bin/mariadb-query-digest --no-continue-on-error --type tcpdump $trunk/t/mariadb-query-digest/samples/bad_tcpdump.txt 2>&1`;
unlike(
   $output,
   qr/Query 1/,
...
$output = `$trunk/bin/mariadb-query-digest --type tcpdump $trunk/t/mariadb-query-digest/samples/bad_tcpdump.txt 2>&1`;
like(
   $output,
...
# #############################################################################
# Infinite loop in mariadb-query-digest if a report crashe
# https://bugs.launchpad.net/percona-toolkit/+bug/888114
# #############################################################################

# This bug is due to the fact that --continue-on-error is on by default.
# To reproduce the problem, we must intentionally crash mariadb-query-digest
