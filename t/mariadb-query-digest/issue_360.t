...
my $pid_file = "/tmp/mariadb-query-digest-test-issue_360.t.$PID";

# Need a clean query review table.
$sb->create_dbs($dbh, [qw(test percona_schema)]);

# Run mariadb-query-digest in the background for 2s,
# saving queries to test.query_review.
diag(`$trunk/bin/mariadb-query-digest --processlist h=127.1,P=12345,u=msandbox,p=msandbox --interval 0.01 --create-review-table --review h=127.1,P=12345,u=msandbox,p=msandbox,D=test,t=query_review --daemonize --pid $pid_file --log /dev/null --run-time 2`);
