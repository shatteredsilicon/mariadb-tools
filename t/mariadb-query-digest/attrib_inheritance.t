...
# #############################################################################
ok(
   no_diff($run_with.'slow034.txt --no-report --output slowlog', "t/mariadb-query-digest/samples/slow034-inheritance.txt"),
   'Analysis for slow034 with inheritance'
);

# Make sure we can turn off some default inheritance, 'ts' in this test.
ok(
   no_diff($run_with.'slow034.txt --no-report --output slowlog --inherit-attributes db', "t/mariadb-query-digest/samples/slow034-no-ts-inheritance.txt"),
