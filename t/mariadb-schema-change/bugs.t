...
# mariadb-schema-change 2.2.17 adds --null-to-not-null feature
# ############################################################################
...
# mariadb-schema-change 2.1.1 doesn't choose the PRIMARY KEY
# ############################################################################
...
# ############################################################################
{
   my $o = new OptionParser(file => "$trunk/legacy-bin/pt-table-checksum");
   $o->get_specs();
   ...
   like(
      $EVAL_ERROR,
      qr/Error copying rows at chunk.*because MariaDB chose/,
      "Dies if MariaDB isn't using the chunk index"
   );
...
# mariadb-schema-change sets bad DELETE trigger when changing Primary Key
# ############################################################################
...
# #############################################################################
# Something like http://bugs.MariaDB.com/bug.php?id=45694 means we should not
# use LOCK IN SHARE MODE with MariaDB 5.0.
# #############################################################################
...
if ( $sandbox_version eq '5.0' ) {
   unlike(
      ...
      "No LOCK IN SHARE MODE for MariaDB $sandbox_version"
   );
}
else {
   like(
      $output,
      qr/LOCK IN SHARE MODE/,
      "LOCK IN SHARE MODE for MariaDB $sandbox_version",
   );
}
...
# mariadb-schema-change error copying rows: Undefined subroutine
...
#    }
# which is called when copying rows causes a MariaDB warning
# for the first time.  So to test this code path, we need to
# cause a MariaDB warning while copying rows.
