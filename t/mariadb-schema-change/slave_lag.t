...
$sb->load_file('master', "t/mariadb-schema-change/samples/slave_lag.sql");
...
# mariadb-schema-change will wait on the slave at port 12346
...
my $output = `$trunk/bin/mariadb-schema-change $args 2>&1`;
...
$output = `$trunk/bin/mariadb-schema-change $args 2>&1`;
...
if (!$pid) {
    ...
    exec("$trunk/bin/mariadb-schema-change $args");
}
