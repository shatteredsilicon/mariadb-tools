...
diag(`$trunk/sandbox/stop-sandbox 12345 >/dev/null`);
diag(`MODE_ANSI=1 $trunk/sandbox/start-sandbox master 12345 >/dev/null`);

my $dp  = new DSNParser(opts=>$dsn_opts);
my $sb  = new Sandbox(basedir => '/tmp', DSNParser => $dp);
my $dbh = $sb->get_dbh_for('master1');
$sb->_d("Sandbox Version: $sandbox_version");

if ( !$dbh ) {
   plan skip_all => 'Cannot connect to sandbox master 12345';
}
...
# ############################################################################
# mariadb-schema-change doesn't work with ANSI_QUOTES + some other sql_modes
# https://bugs.launchpad.net/percona-toolkit/+bug/1058285
# ############################################################################
$sb->load_file('master1', "$sample/sql-mode-bug-1058285.sql");

$dbh->do("SET GLOBAL SQL_MODE=ANSI"); # THIS IS MARIADB!
$dbh->disconnect;
$dbh = $sb->get_dbh_for('master1');
...
SKIP: {
    ...
    # ############################################################################
    # mariadb-schema-change foreign key error
